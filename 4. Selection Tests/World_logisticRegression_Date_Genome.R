...
# Functions
# =============================================================================
log.reg.model <- function(cur.snp, date)
{
  tryCatch(
    {
      ...
      cur.log.model <- stats::glm(snp ~ date, data = cur.data, family = "binomial")
...
# =============================================================================
# Africa 
log.reg.africa.pnpla3 <- as.data.frame(lapply(africa.ind.pnpla3[c(9:length(individuals.data))], log.reg.model, africa.ind.pnpla3$date))
log.reg.africa.pnpla3 <- log.reg.africa.pnpla3[colSums(!is.na(log.reg.africa.pnpla3)) > 0]
# America 
log.reg.america.pnpla3 <- as.data.frame(lapply(america.ind.pnpla3[c(9:length(individuals.data))], log.reg.model, america.ind.pnpla3$date))
log.reg.america.pnpla3 <- log.reg.america.pnpla3[colSums(!is.na(log.reg.america.pnpla3)) > 0]
# Asia 
log.reg.asia.pnpla3 <- as.data.frame(lapply(asia.ind.pnpla3[c(9:length(individuals.data))], log.reg.model, asia.ind.pnpla3$date))
log.reg.asia.pnpla3 <- log.reg.asia.pnpla3[colSums(!is.na(log.reg.asia.pnpla3)) > 0]
# Europe 
log.reg.europe.pnpla3 <- as.data.frame(lapply(europe.ind.pnpla3[c(9:length(individuals.data))], log.reg.model, europe.ind.pnpla3$date))
...
# =============================================================================
# logistic Regression Genome World
# =============================================================================
# =============================================================================
...
ggsave(filename = "LogisticRegression_Date_Genome.pdf", plot = log.reg.gen.world, device = "pdf", path = "5. Plots", width = 18, height = 26, units = "cm")
ggsave(filename = "LogisticRegression_Date_Genome.png", plot = log.reg.gen.world, device = "png", path = "5. Plots", width = 18, height = 26, units = "cm")
